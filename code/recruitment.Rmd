...
teacher_year_analysis %<>% 
  mutate(pie_hire = case_when(t_newhire == 0 ~ "Experienced Teachers",
...
teacher_year_analysis %>% 
  ...
  ggplot(aes(x = " ", y = n, fill = pie_hire)) + 
    ...
    labs(title = "Share of Teacher Who Are New Hires", 
         fill = NULL) + 
    theme_tntp_2018(grid = FALSE,
                    axis_text = FALSE)
...
teacher_year_analysis %<>% 
  mutate(t_newhire_type = case_when(t_newhire == 1 & t_novice == 1 ~ "Novice New Hire",
...
# // Step 5:  Calculate counts table.
tabyl_school_year_t_newhire_type_n <- teacher_year_analysis %>% 
  tabyl(school_year, t_newhire_type) %>% 
  gather(type, n, -school_year)

# // Step 6:  Calculate percentages table.
tabyl_school_year_t_newhire_type_perc <- teacher_year_analysis %>% 
  tabyl(school_year, t_newhire_type) %>% 
  adorn_percentages("row") %>% 
  gather(type, percent, -school_year)

# // Step 7:  Calculate significance indicator variables by year.
...
# 
# tabyl_school_year_t_newhire_type_sig <- chisq.test(tabyl(school_year, t_newhire_type))$stdres %>% # or is it residuals
#   mutate_at(vars(-school_year), ~ ifelse(. > 1.96, "*", "")) %>% 
#   mutate(school_year = as.double(school_year))
# 
# # // Step 8:  Concatenate values and significance asterisks to make value labels.
# tabyl_school_year_t_newhire_type <- tabyl_school_year_t_newhire_type_perc %>% 
#   left_join(tabyl_school_year_t_newhire_type_sig, by = "school_year", suffix = c("", "_sig")) %>% 
#   mutate(`Experienced New Hire_label` = paste0(scales::percent(`Experienced New Hire`, accuracy = 1), `Experienced New Hire_sig`),
#          `Novice New Hire_label` = paste0(scales::percent(`Novice New Hire`, accuracy = 1), `Novice New Hire_sig`))
# 
# # // Step 9:  Create a stacked bar graph using overlaid bars.
# tabyl_school_year_t_newhire_type %>% 
#   gather(type, percent, `Novice New Hire`:`Returned Teachers`) %>% 
#   gather(type, label, `Experienced New Hire_label`:`Experienced New Hire_label`) %>% 
#   ggplot(aes(x = school_year, y = percent, fill = type)) + 
#     geom_bar(stat = "identity")
# 
...
tabyl_t_race_ethnicity <- teacher_year_analysis %>% 
  ...
  select(race_ethnicity = t_race_ethnicity, 
         t_total = percent)
...
tabyl_s_race_ethnicity <- student_teacher_year_analysis %>% 
  ...
  select(race_ethnicity = s_race_ethnicity, 
         s_total = percent)
...
race_ethnicity_tabyl <- race_ethnicity_tabyl %>% 
  ...
  mutate(race_ethnicity = case_when(race_ethnicity == 1 ~ "Black", 
